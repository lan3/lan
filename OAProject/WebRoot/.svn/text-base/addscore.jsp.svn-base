...
<body>
    ...
    <form action="score!add.action" method="post">
    <ul class="forminfo">
    <li><label>学生姓名</label>
    	<input name="sc.sid" type="text" class="dfinput" /><i></i></li>
    <li><label>所在班级</label>
    	<select name="sc.classid">
			<c:forEach items="${listclass}" var="class">
				<option value="${class.classname }">${class.classname }</option>
			</c:forEach>
		</select><i></i></li>
    <li><label>考试科目</label><cite>
    	<select name="sc.onjectname">
    ...
    <li><label>总分</label>
    	<input name="sc.score" type="text" class="dfinput"/></li>
