...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	$(".select3").uedSelect({
		...
		$(".save").click(function(){  //点击保存事件
			location.href="patrolshuse!test.action?element="+getdata();
		});
		});
        ...
        function del(){
	        ...
	        for(var i=0;i<=qcheck.length-1;i++){
	       	...
	       	if(datas==""){
	       		...
	       		location.href="patrolshuse!dels.action?dids="+datas;
	       	}
         ...
         function tosel1(){
         	...
         	location.href="patrolshuse!seldata1?hname="+hname+"&hnum="+hnum;
         }
         function tosel2(){
         	var t1=document.getElementById("d11").value;
         	var t2=document.getElementById("d12").value;
         	location.href="patrolshuse!seldata2?t1="+t1+"&t2="+t2;
         }
</script>
	</head>
	<body onload="gehang()">
		...
		<div class="rightinfo">
			<div class="tools">
				<ul class="seachform">
					<li>
						...
						<div class="vocation">
						<select id="hname"  class="select3" >
							<option>--选择宿舍--</option>
							<c:forEach items="${hlist}" var ="h">
								<option value="${h.HOURSEAT }">${h.HOURSEAT }</option>
							</c:forEach>
						</select>
						</div>
						<div class="vocation">
						<select onchange="tosel1();" id="hnum"  class="select3" >
							<option>--门牌号--</option>
							<c:forEach items="${hslist}" var ="num">
								<option value="${num.HOURMARK }">${num.HOURMARK }</option>
							</c:forEach>
						</select>
						</div>
					</li>
			...
			<table class="tablelist" id="tabProduct">
				<tr>
					...
					<td EditType="DropDownList"
					DataItems="<c:forEach items="${hlist}" var="h">
								{text:'${h.HOURSEAT }',value:'${h.HOURSEAT }'},
								</c:forEach>">
						单元号
					</td>
					<td EditType="DropDownList"
					DataItems="<c:forEach items="${hslist}" var="h">
								{text:'${h.HOURMARK }',value:'${h.HOURMARK }'},
								</c:forEach>">
						寝室号
					</td>
					<td EditType="TextBox">
				...
				<c:forEach items="${list}"  var="patorls">
					<tr>
						<td>
							<input name="check[]" value="${patorls.PSID }"  type="checkbox" />
						</td>
						<td>${patorls.PSID }</td>
						<td><c:forEach items="${emplist}" var="emp">
							${emp.EID==patorls.EID?emp.ENAME:'' }
						</c:forEach></td>
						<td><fmt:formatDate value="${patorls.HDATE }" pattern="yyyy-MM-dd"/></td>
						<td>${patorls.HTIME }</td>
						<td>${patorls.HOURSENAME }</td>
						<td>${patorls.HOURSENUM }</td>
						<td>${patorls.YCOUNT }</td>
						<td>${patorls.SCOUNT }</td>
						<td>${patorls.HDESC }</td>
						<td>${patorls.HADVICE }</td>
						<td>
							无
						</td>
					</tr>
...
function AddRow(table, index){
    ...
    location.href="patrolshuse!adds.action?element="+data;
	return newRow;
}
