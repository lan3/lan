...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	$(".select3").uedSelect({
		...
		$(".save").click(function(){  //点击保存事件
			location.href="dept!test.action?element="+getdata();
		});
		});
        ...
        function del(){
	        ...
	        for(var i=0;i<=qcheck.length-1;i++){
	       	...
	       	if(datas==""){
	       		...
	       		location.href="patrol!dels.action?dids="+datas;
	       	}
          ...
          function tosel1(){
         	...
         	location.href="patrol!seldata1?t1="+t1+"&t2="+t2;
         }
         function tosel2(){
         	var peid=document.getElementById("empename").value;
         	location.href="patrol!seldata2?peid="+peid;
         }
         function tosel3(){
         	var name=document.getElementById("cname").value;
         	location.href="patrol!seldata3?name="+name;
         }
</script>
	</head>
	<body onload="gehang()">
		...
		<div class="rightinfo">
			<div class="tools">
				<ul class="seachform">
					<li>
						...
						<div class="vocation">
						<select onchange="tosel2();" id="empename"  class="select3" >
							<option>--选择员工--</option>
					...
					<li>
						...
						<div class="vocation">
						<select onchange="tosel3();" id="cname"  class="select3" >
							<option>--选择班级--</option>
			...
			<table class="tablelist" id="tabProduct">
				<tr>
					...
					<td EditType="DropDownList" DataItems="{text:'08:00:00',value:'08:00:00'},{text:'11:00:00',value:'11:00:00'},{text:'14:00:00',value:'14:00:00'},{text:'16:00:00',value:'16:00:00'},{text:'19:00:00',value:'19:00:00'},{text:'20:00:00',value:'20:00:00'}">
						值班时间
					</td>
					<td  EditType="DropDownList"
					DataItems="<c:forEach items="${clist}" var="class">
								{text:'${class.CLASSNAME }',value:'${class.CLASSNAME }'},
								</c:forEach>"
					>班级</td>
					<td>
						应到人数
					...
					<td EditType="DropDownList"  DataItems="{text:'好',value:'好'},{text:'一般',value:'一般'},{text:'差',value:'差'}">
						教室卫生
					</td>
					<td EditType="DropDownList"
					DataItems="<c:forEach items="${tlist}" var="teacher">
								{text:'${teacher.EMPTEACH }',value:'${teacher.EMPTEACH }'},
								</c:forEach>">
						任课老师
					</td>
					<td EditType="DropDownList" DataItems="{text:'是',value:'是'},{text:'否',value:'否'}">
						任课老师是否到岗
					</td>
					<td EditType="TextBox">
						学生违纪违规情况
				...
				<c:forEach items="${plist}" var="patorl">
					<tr>
						<td>
							<input name="check[]" value="${patorl.PID }"  type="checkbox" />
						</td>
						<td>${patorl.PID }</td>
						<td>
							<c:forEach items="${emplist}" var="emp">
							${emp.EID==patorl.EID?emp.ENAME:'' }
						</c:forEach>
						</td>
						<td><fmt:formatDate value="${patorl.PDATETIME }" pattern="yyyy-MM-dd"/></td>
						<td>${patorl.PTIME }</td>
						<td>${patorl.PCLASS }</td>
						<td>${patorl.YCOUNT }</td>
						<td>${patorl.SCOUNT }</td>
						<td>${patorl.WCDESC }</td>
						<td>${patorl.TNAME }</td>
						<td>${patorl.TISSTAY }</td>
						<td>${patorl.STUDESC }</td>
						<td>${patorl.TISDO }</td>
						<td>
							无
						</td>
...
function AddRow(table, index){
    ...
    location.href="patrol!adds.action?element="+data;
	return newRow;
}
