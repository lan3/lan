        ...
        if(type=="qx"){
         ...
         function seldata4(){
			...
			var num=new Array();
     		...
     		var msg= prompt("请输入保存的文件名称","");  
         	location.href="report!excel?data="+num+"&filename="+msg;
   }
</script>
	</head>
	<body onload="gehang()">
		<div class="place">
			...
			<ul class="placeul">
				...
				<li>
					<a href="report!sellist.action#">查看工作日志</a>
				</li>
			</ul>
		</div>
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="seachform">
					...
					<li>
						<label>
							<input type="button"  onclick="seldata3();" class="btn" value="查看本周"  />
						</label>
					</li>
					<li>
						<label>
							<input type="button"  onclick="seldata4();" class="btn" value="导出Excel"  />
						</label>
					</li>
				</ul>
			...
			<table class="tablelist" id="tabProduct">
				...
				<c:forEach items="${list}" var="report">
					<tr>
						<td>
							<input name="check[]" value="${report.RID }" type="checkbox" />
						</td>
						<td>${report.RID }</td>
						<td>
						<c:forEach items="${emplist}" var ="emp">
							${report.EID==emp.EID?emp.ENAME:'' }
						</c:forEach>
						</td>
						<td>
			...
			<ul class="paginList">
				<li class="paginItem">
					<a href="report!list.action?page=${currpage-1 }"><span
						class="pagepre"></span> </a>
				</li>
				<c:forEach begin="1" end="${totalpage}" var="i">
					<li class="paginItem">
						<a href="report!list.action?page=${i }">${i }</a>
					</li>
				</c:forEach>
...
function AddRow(table, index){
    ...
    location.href="report!adds.action?element="+data;
	return newRow;
}
