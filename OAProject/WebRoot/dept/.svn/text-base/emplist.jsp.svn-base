		...
		$(document).ready(function(){
		  ...
		  $(".tip").fadeOut(100);
			...
			$(".add").click(function(){  //点击add事件
				location.href="emp!seldept.action";
			});
        ...
        function del(){
	        ...
	        for(var i=0;i<=qcheck.length-1;i++){
	       	...
	       	if(datas==""){
	       		...
	       		location.href="emp!dels.action?dids="+datas;
	       	}
		...
		<div class="rightinfo">
			...
			<table class="tablelist" id="tabProduct">
				...
				<c:forEach items="${list}" var="emp">
					<tr>
						<td>
							<input name="check[]" value="${emp.EID }" type="checkbox" />
						</td>
						<td>${emp.EID }</td>
						<td>${emp.ENAME }</td>
						<td>${emp.SEX }</td>
						...
						<td>${emp.EJOB }</td>
						<td>
							<a href="emp!upd.action?emp.eid=${emp.EID }" class="tablelink">查看</a>
							<a onclick="return confirm('确认要删除吗？')"
								href="emp!del.action?emp.eid=${emp.EID }" class="tablelink">
								删除</a>
						</td>
					</tr>
				</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					...
					<c:forEach begin="1" end="${totalpage}" var="i">
						<li class="paginItem">
							<a href="emp!list.action?page=${i }">${i }</a>
						</li>
					</c:forEach>
					<li class="paginItem">
						<a href="emp!list.action?page=${currpage+1 }"><span
