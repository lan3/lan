						...
						<tr>
							...
							<td>
								...
								<c:if test="${t.STARTS=='审核中' }">
									<a href="leve!teachesty.action?s.leveid=${t.LEVEID }&sid=${t.SID}&classid=${t.CLASSID }&startdate=${t.STARTDATE }&endtime=${t.ENDTIME }&day=${t.DAY }&empteach=${t.DAY <= 1? '无':t.ENAME }&lcont=${t.LCONT }&starts=${t.STARTS }" class="tablelink">同意</a>
									<a href="leve!teachesnty.action?s.leveid=${t.LEVEID }" class="tablelink">不同意</a>
								</c:if>
								<c:if test="${t.STARTS=='审核成功'}">
									审核通过
								</c:if>
								<c:if test="${t.STARTS=='最终审核'}">
									${t.STARTS }
								</c:if>
