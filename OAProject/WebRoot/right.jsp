...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(".add").click(function(){
			AddRow(document.getElementById('tabProduct'),1);
		});
		$(".del").click(function(){  //点击删除事件
		});
		$(".save").click(function(){  //点击保存事件
			location.href="dept!test.action?element="+getdata()+"&last="+getlastdata();
		});
        ...
        function del(){
	        ...
	        for(var i=0;i<=qcheck.length-1;i++){
	       	...
	       	if(datas==""){
	       		...
	       		location.href="dept!dels.action?dids="+datas;
	       	}
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="toolbar">
					...
					<li class="save">
						<span><img src="images/t01.png" /> </span>保存
					</li>
					<li class="click">
			...
			<table class="tablelist" id="tabProduct">
				<tr>
					...
					<td width="155px" Name="Num">
						编号
						<i class="sort"><img src="images/px.gif" /> </i>
					</td>
					<td width="155px" EditType="DropDownList" DataItems="{text:'教学部',value:'教学部'},{text:'行政部',value:'行政部'},{text:'后勤部',value:'后勤部'},{text:'招生部',value:'招生部'},{text:'财务部',value:'财务部'}">
						部门名称
					</td>
					
					<td width="155px" EditType="DropDownList" 
					...
					<td width="155px" EditType="DropDownList" DataItems="{text:'可用',value:'可用'},{text:'不可用',value:'不可用'}">
						状态
					</td>
					<td width="155px" EditType="TextBox">
						创建时间
					</td>
					<td width="155px" EditType="DropDownList"
					DataItems="<c:forEach items="${emplist}" var="emp">
				...
				<c:forEach items="${list}" var="dept">
					<tr>
						...
						<td>${dept.status=='1'?'可用':'不可用' }</td>
						<td>${dept.createtime }</td>
						<td>
						<c:forEach items="${emplist}" var ="emp">
							${dept.xid==emp.EID?emp.ENAME:'' }
						</c:forEach></td>
						<td>
							<a onclick="return confirm('确认要删除吗？')"
								href="dept!del.action?dept.did=${dept.did }" class="tablelink">
								删除</a>
						</td>
					</tr>
				</c:forEach>
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem">
						<a href="dept!list.action?page=${currpage-1 }"><span
							class="pagepre"></span> </a>
					</li>
					<c:forEach begin="1" end="${totalpage}" var="i">
						<li class="paginItem">
							<a href="dept!list.action?page=${i }">${i }</a>
						</li>
					</c:forEach>
					<li class="paginItem">
						<a href="dept!list.action?page=${currpage+1 }"><span
			...
			<div class="tip">
				...
				<div class="tipbtn">
					<input name="" type="button" onclick="del();" class="sure"
						value="确定" />
					&nbsp;
...
function CreateTime(element){
	...
	if(editState != "true"){
		...
		textBox.type = "text";
		textBox.className="EditCell_TextBox";
		//textBox.onclick="WdatePicker()";
		//textBox.onclick=eval("WdatePicker()");
		//textBox.click("WdatePicker()");
...
function AddRow(table, index){
  ...
  //        		data.push(table.rows[i].cells[j+1].innerHTML);
   //       }  
    //}
   // location.href="dept!adds.action?element="+data;
	//return newRow;
}
