					...
					<c:forEach items="${dept}" var="d">
						<tr>
							...
							<td>
								...
								<c:if test="${d.STARTS=='最后审核' }">
									<a href="com!deptcty.action?c.coid=${d.COID }&sids=${d.SIDS }&classid=${d.CLASSID }&times=${d. TIMES}&lconts=${d.LCONTS }&starts=${d.STARTS}" class="tablelink">同意</a>
									<a href="com!deptsnoty.action?c.coid=${d.COID }" class="tablelink">不同意</a>
								</c:if>
								<c:if test="${d.STARTS=='审核成功'}">
									审核成功
								</c:if>
								<c:if test="${d.STARTS=='审核失败'}">
									审核失败
								</c:if>
							</td>
						</tr>
